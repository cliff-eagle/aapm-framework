...
The influence of a learner's native language (L1) on their target language (L2) production. Transfer can be **positive** (L1 structures that work in L2) or **negative** (L1 structures that produce errors in L2). The Interlanguage Hypothesis Engine tracks transfer patterns and adjusts predictions accordingly.
...
The architectural principle that no single LLM vendor is required. The AAPM is designed for provider agnosticism — any model supporting structured output, function calling, and embeddings can serve as the inference backend. Cost benchmarks and capability mappings are maintained in `model-substitution.md`.
...
A Tier 3 failure state where the learner's communicative breakdown is severe enough that the interaction cannot continue. Triggers the Negotiation Collapse Recovery protocol: a structured de-escalation → repair → re-engagement sequence designed to convert failure into learning without triggering the affective filter.
...
A planned community-driven ecosystem for creating, sharing, validating, and distributing persona schemas. Enables domain experts to publish schemas without code — expanding the AAPM's coverage to any learning domain. See `schema-marketplace.md`.
...
A graceful recalibration procedure triggered when the system detects a learner has been placed above their actual proficiency level. Instead of abruptly downgrading difficulty (which would trigger the affective filter), the protocol gradually reduces communicative pressure while maintaining the illusion of natural challenge variation. See `soft-landing-protocol.md`.
...
The explainability architecture providing three audiences — learners, educators, and auditors — with appropriate visibility into AAPM decision-making. Learners see progress narratives; educators see dashboards with intervention rationale; auditors see algorithm documentation and bias analysis. See `transparency-layer.md`.
